...
let run ~limits ~proof_file ?remote_info prover pb : _ Run_result.t =
  ...
  (* spawn process *)
  let raw =
    Prover.run ?proof_file ~limits ~file:pb.Problem.name ?remote_info prover
  in
  let result =
...
let run_proof_check ~limits ?remote_info prover checker pb ~proof_file : check_res =
  ...
  let raw =
    Proof_checker.run ~limits ~problem:pb.Problem.name ~proof_file ?remote_info checker
  in
