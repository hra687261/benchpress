...
module Exec_run_provers : sig
  ...
  let expand ?(slurm = false) ?j ?(dyn = false) ?limits ?proof_dir ?interrupted
      defs s_limits s_j s_pattern s_dirs s_provers : expanded =
    let limits =
      match limits with
      | None -> s_limits
  ...
  let run ?(timestamp = Misc.now_s ()) ?(on_start = _nop) ?(on_solve = _nop)
      ...
      ?(interrupted = fun _ -> false) ~uuid ~save (self : expanded) : _ * _ =
  ...
  let run_sbatch_job ?(timestamp = Misc.now_s ()) ?(on_start = _nop)
      ...
      ~port ~ntasks ~uuid ~save (self : expanded) : _ * _ =
    ...
    in
    Log.debug (fun k ->
        k "Submitted %d scripts to launch workers to slurm."
          (List.length job_ids));
    ...
    List.iter
      ...
      job_ids;
    Log.debug (fun k ->
        k "Killed the workers that are still alire or didn't start yet.");

    Sys.remove config_file;
    Sys.remove resp_sock_path;
    Log.debug (fun k ->
        k "Delete the generated config_file and linux socket file.");
...
module Git_checkout = struct
  ...
  let run (self : t) : unit =
    ...
    | _ -> ());
    with_chdir dir (fun () -> run_cmd ("git checkout " ^ ref))
end

(** Run the given action *)
let rec run ?(save = true) ?interrupted ?cb_progress (defs : Definitions.t)
    (a : Action.t) : unit =
  ...
  | Action.Act_run_provers r ->
    ...
    let _top_res, res =
      Exec_run_provers.run ?interrupted ~on_solve:progress#on_res
        ...
        ~save ~timestamp:(Misc.now_s ()) ~uuid r_expanded
    in
  ...
  | Act_run_slurm_submission
    ...
    Format.printf "task done: %a@." Test_compact_result.pp res;
    ()
  | Action.Act_progn l -> List.iter (fun a -> run ~save ?interrupted defs a) l
