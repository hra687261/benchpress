  ...
  let cmd =
    ...
    let aux j pp_results dyn paths dir_file proof_dir defs task timeout memory
      ...
      Run_main.main ~pp_results ?dyn ~j ?timeout ?memory ?csv ~provers ~meta
        ?task ?summary ?dir_file ?proof_dir ~save defs paths ()
    in
...
module Slurm = struct
  ...
  let cmd =
    let open Cmdliner in
    let aux j pp_results dyn paths dir_file proof_dir defs task timeout memory
        meta provers csv summary no_color partition nodes addr port ntasks =
      catch_err @@ fun () ->
      if no_color then CCFormat.set_color_default false;
      let dyn =
        if dyn then
      ...
      Run_main.main ~sbatch:true ~pp_results ?dyn ~j ?timeout ?memory ?csv
        ~provers ?summary ~meta ?task ?dir_file ?proof_dir
