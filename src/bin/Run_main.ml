...
let execute_run_prover_action ?j ?timestamp ?pp_results ?dyn ?limits ?proof_dir
    ~notify ~uuid ~save (defs : Definitions.t) (r : Action.run_provers) :
...
let execute_submit_job_action ?pp_results ?j ?timestamp ?dyn ?limits ?proof_dir
    ~notify ~(uuid : Uuidm.t) ~(save : bool) (defs : Definitions.t)
    (r : Action.run_provers_slurm_submission) : _ * Test_compact_result.t =
  Error.guard
    (Error.wrapf "run prover slurm action@ `@[%a@]`" Action.pp_run_provers_slurm
       r)
  @@ fun () ->
  let interrupted = CCLock.create false in
  let exp_r =
  ...
  let result =
    ...
    Exec_action.Exec_run_provers.run_sbatch_job ~uuid ?timestamp
      ...
      ~ntasks:r.ntasks ~save ~on_solve:progress#on_res
      ~on_start_proof_check:(fun () -> progress#on_start_proof_check)
      ~on_proof_check:progress#on_proof_check_res
      ~on_done:(fun _ -> progress#on_done)
      exp_r
...
let main ?j ?pp_results ?dyn ?timeout ?memory ?csv ?(provers = []) ?meta:_
    ?summary ?task ?dir_file ?proof_dir ?(save = true) ?(sbatch = false)
    ?partition ?nodes ?addr ?port ?ntasks (defs : Definitions.t) paths () : unit
    =
  Log.info (fun k ->
  ...
  let tt_task =
    ...
    | Some task_name ->
      ...
      | { view = { Task.action = Action.Act_run_provers r; _ }; loc }
        ...
        let r =
          let r_ =
            Definitions.mk_run_provers_slurm_submission ?partition ?nodes ?j
              ?addr ?port ?ntasks ~paths ?timeout ?memory ~provers ?loc:r.loc
              defs
          in
          {
            r_ with
  ...
  in
  match tt_task with
  | TT_other a -> Exec_action.run ~save defs a
  | TT_run_provers (run_provers_action, defs) ->
    ...
    let top_res, (results : Test_compact_result.t) =
      execute_run_prover_action ~uuid ?pp_results ?proof_dir ?dyn:progress
        ~limits ?j ~notify ~timestamp ~save defs run_provers_action
    in
