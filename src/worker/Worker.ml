...
module Log =
  (val Logs.src_log (Logs.Src.create "benchpress-worker"))
...
let run_prover_pb ?proof_dir ~limits ~prover ~pb provers checkers =
  let prover =
    ...
    with Not_found ->
      Error.failf "cannot find prover '%s'" prover
  in
  Error.guard
    (Error.wrapf "(@[worker: running :prover %a :on %a@])"
  ...
  let result =
    Run_prover_problem.run ~limits ~proof_file
      prover pb
  in
  let ev_proof =
  ...
  try
    ...
    while true do
      ...
      | Msg.Worker_task cmds ->
        Log.debug (fun k->k"(@[run_worker %d: received %d tasks@])"
                      id (List.length cmds));
        let reps =
          List.flatten @@
          Misc.Par_map.map_p ~j (
            fun (prover, pb) ->
              run_prover_pb ?proof_dir:None ~limits ~prover ~pb
                provers checkers
          ) cmds
        in
        Log.debug (fun k->k"(@[run_worker %d: sent %d responses@])"
                      id (List.length reps));
        Marshal.to_channel oc (Msg.Worker_response (id, reps)) [];
        flush oc
      | Msg.Stop_worker -> raise Stop
    done
  with
  | Stop ->
    Log.debug (fun k->k"(@[run_worker %d: received \"Stop\" message from \
                        master@])" id);
    close_out oc
  | End_of_file ->
    Log.debug (fun k->k"(@[run_worker %d: master closed connection \
                        \"End_of_file\"@])" id);
    close_out oc
  | e ->
