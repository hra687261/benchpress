...
let parse_cmdline =
  ...
  let aux defs id socket_addr_opt socket_port j timeout memory =
    try
      ...
      true
    with
    | Error.E e -> Error.failf "%a@." Error.pp e
  in
  let defs = Bin_utils.definitions_term
  and id =
    Arg.(value & opt int (-1) & info ["i"; "id"] ~doc:"unique stricly positive integer worker ID")
  and j =
    Arg.(value & opt int (Misc.guess_cpu_count ()) & info ["j"] ~doc:"level of parallelism")
  and socket_addr_opt =
    Arg.(value & opt (some Misc.ip_addr_conv) None & info ["a"; "addr"] ~doc:"IP socket address")
  and socket_port =
  ...
  Cmd.v (Cmd.info ~doc "run")
    (Term.(const aux $ defs $ id $ socket_addr_opt $ socket_port $ j $
           timeout $ memory))
